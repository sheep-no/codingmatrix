  ...
  if (workspace) {
    ...
    void authorization.grant(workspace.name, workspace.uri.fsPath).then(() => {
      session.acceptHandshake({
        ...
        protocol_version: 1,
        capabilities: ["workspace", "file", "terminal", "diagnostics", "validation"],
        policy_version: 1,
        policy,
        pending_actions: [],
      });
      ...
      runtime = new AgentHostRuntime({ session, dispatcher, approvalBridge: approval, onEvent: (event) => controller.publish(event) });
    });
  }
