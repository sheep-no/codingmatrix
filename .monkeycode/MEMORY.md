...
- Instructions:
  ...
  - `POST /api/v1/agent/orchestrate/stream` 有磁盘守卫：可用空间 <1GB 或可用率 <10% 直接返回 507（`app/utils/guardrails.py:250`），且会写 `./projects`。跑 agent 端到端生成前需 >2GB 可用，本机根分区 20G、常年 92% 占用，需先腾空间。腾空间优先删纯构建产物与可重下载缓存：`flutter_client/build/app/intermediates`、`build/test_cache`、`/root/.gradle/caches`（保留 `build/linux` 的 release bundle 与 `build/app/outputs` 的 APK 作证据），实测可用由 788MB 升到 4.5GB。
  - 编排端到端要跑到 `done`，需求与产物须避开 `unknown` 文件类型：`.py` 在 `EXTENSION_TYPE_MAP` 无兜底（`app/agent/dependency_rules.py:189`），非常规命名的 `.py` 会被推断为 `unknown`，`_infer_unknown_file_types` 随即 `RuntimeError` 中止整轮（`spec_first_generate.py:1955`）。用 `.html/.css/.js/.md` 这类有兜底扩展名的静态站需求即可稳定跑到 `done`（实测 4/4 文件、`success=true`）。真实运行时架构师角色是 `Qwen/Qwen3-8B`（`data/unified_model_config.yaml:356`），弱模型对简单需求会幻觉出 Web 架构或多余文件（如静态站里生成 `app/command.py`）。架构师还可能完全不返回 `project_spec`（该模型实测稳定缺失），`app/agent/architect.py:364`/`:689` 现已改为空兜底 `{"default": {"terminology": {}}}` 并打 warning，不再硬失败。legacy 新建整轮实测约 9-11 分钟；插件 E2E 宿主 harness（`/tmp` 下的 `host.suite.mjs` 模板）的等待上限必须大于生成耗时，否则会在生成结束前退出造成误判。访问令牌 `ACCESS_TOKEN_EXPIRE_MINUTES=30`，跑长流程前先重新登录刷新。编排流的 `thinking` 事件除增量 `message` 外还带一份全量累积 `accumulated`（真实 legacy 流 88MB 中占 66MB），插件已在 SSE 解析处剥离；判定 SSE 事件字段形状要取全部样本的字段并集，首个样本可能缺字段。
  ...
  - webview 面板打开后不会立刻出现在 `vscode.window.tabGroups.all` 里：`executeCommand` 返回时标签页可能尚未创建，直接断言会得到 0 个标签。要轮询等待（实测约 250ms 内出现），否则会把时序问题误判成「面板没打开」。真实宿主内逐页验收用 `--remote-debugging-port` + CDP（Node 全局 WebSocket 即可，无需 playwright）：扩展 webview 在 CDP 里是一个 `type=iframe` 目标，但它的 `document` 不含工作台标记，标记在其内部 `fake.html` 子 iframe 的 `contentDocument` 里，必须穿一层才能读到 `.tab-panel`；截图用 `Page.captureScreenshot` 并以 webview 元素矩形为 `clip`。`src/workbench-html.ts` 的内联脚本整体是模板字符串：正文里不能出现反引号或 `${`，正则反斜杠必须写成双反斜杠（`\\s`、`\\n`），否则会被模板转义折叠（`\s` 变成字面 `s`、`\n` 变成真实换行）；改完务必从 `createAgentWorkbenchHtml()` 的输出核对实际发出的脚本。
  ...
  - e2e 首次运行由 `@vscode/test-electron` 下载 VS Code（实测 1.139.0，325.98 MB）到 `.vscode-test/`，解压后约 971 MB，会把磁盘压到 97%；缓存带 `is-complete` 标记，重跑直接复用，测完可删；下载中断留下的无 `is-complete` 目录不可用且会占满磁盘。可用 `runTests({ vscodeExecutablePath })` 直接指向已缓存的可执行文件，跳过联网下载与版本解析。`stable` 解析到的最新版会与缓存版不一致（实测缓存 1.139.0 而 `stable`=1.139.1），此时两个入口都改走下载且 CDN 会反复中断该 326MB 包（curl 18 / `Error: aborted`）；可对 `https://update.code.visualstudio.com/<ver>/linux-x64/stable` 断点续传补齐 `.vscode-test/vscode-linux-x64-<ver>` 再 `touch is-complete`，不改仓库代码即复用缓存。宿主 harness 启动 VS Code 必须带 `DISPLAY`（Xvfb `:99` 常驻），缺失时报 `Missing X server or $DISPLAY` 并以 SIGTRAP 结束。删除源码后 tsc 不清理对应的 `dist/*.js|.d.ts|.js.map` 残留，而 `vsce package` 按 `files: ["dist/**"]` 全打包，打包前必须手动清理。VS Code 1.139 启动会打印大量内置子系统日志（`[AgentHost] No signed-in session resolved for https://api.github.com`、`[ChatModelSelection]`、`Unknown channel`、dbus 连接失败），与本插件无关，排查时忽略。
...
- Instructions:
  ...
  - Android 打包需 NDK 27（`flutter build apk` 由 AGP 触发，需 strip native 库），已装在 `/tmp/opencode/android-sdk` 且 `/tmp` 会被清理：重建需 cmdline-tools 11076708 加 `sdkmanager "platform-tools" "platforms;android-36" "build-tools;35.0.0" "ndk;27.0.12077973"`，并设 `JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64`。NDK 解压约 2.0G，APK 构建后根分区只剩约 1.7G 可用；移除 `jni`（pin `path_provider_android`）不能免除该需求，已回滚该覆盖。
  - Android release 构建已实测通过：`flutter build apk --release --no-pub` 增量约 3 分钟、全量约 14 分钟，产出 `build/app/outputs/flutter-apk/app-release.apk`（57.9MB，含 arm64-v8a/armeabi-v7a/x86_64）；只需验 Dart 编译时用 `flutter build bundle --target-platform android-arm64`（快得多），验原生/Kotlin 用 `./gradlew :app:compileDebugKotlin --offline`。核对项：`namespace == Kotlin package == 源码目录路径`，Manifest 用 `.MainActivity` 相对 namespace 解析。环境无设备且模拟器不可用：无 `/dev/kvm`、CPU 无 `vmx`/`svm`、SDK 未装 `emulator` 与 system-image，真机项只能静态核对。无设备静态验收清单（build-tools 36.0.0 工具，全部可离线跑）：`apksigner verify` 验签、`zipalign -c -P 16 -v 4`（`extractNativeLibs=false` 时 Android 15+ 要求 16KB 页对齐，只跑 `-c 4` 不够）、`aapt dump badging` 看 SDK/ABI/权限、`aapt2 dump xmltree AndroidManifest.xml` 看 `allowBackup`/`usesCleartextTraffic`/`debuggable`、`aapt2 dump resources` 看资源变体、确认有 `libapp.so` 且无 `kernel_blob.bin` 为 AOT release。资源改动可先用 `aapt2 compile --dir <res> -o r.zip` + `aapt2 link -R r.zip --manifest <合成最小清单>`（真清单含 `${applicationName}` 占位与缺失 package 会链接失败）+ `aapt2 dump resources` 快速验证解析，不必等整套 Gradle；启动图标必须是 `drawable-anydpi-v26/ic_launcher.xml`（`<adaptive-icon>`），仅给传统 drawable 时 Android 8+ 会缩小并垫系统底板。
  - Android release 签名读取 `android/key.properties` 与同目录 keystore，二者均被 `android/.gitignore` 排除、不入库；该文件缺失时回退 Android debug 签名，所以新克隆仍能 `flutter run --release`。生成密钥与字段格式见 `.monkeycode/docs/PENDING-VERIFICATION.md`「Android APK 构建」；发布前须递增 `pubspec.yaml` 构建号，keystore 丢失则无法更新已发布应用。
  ...
  - 无 keyring 的环境（容器/WSL/纯 WM）会打印 `libsecret_error: Failed to unlock the keyring`：安全存储读写失败会让登录与服务会话全链路不可用（`CloudAuthClient._accept` 里 `saveSession` 抛错即 `logout()`），所以这不是能忽略的噪声。凭据层已把它归类为 `SecureStorageUnavailableException` 并给出对应提示。要在本环境实跑登录，最省事的方式是不用 `dbus-launch`（它会把总线地址写进 X11 根窗口属性，污染同一 DISPLAY），改用 `dbus-run-session -- bash -c 'export HOME=<fresh>; printf "testpass" | gnome-keyring-daemon --unlock --components=secrets; exec ./flutter_client'`：全新 HOME 配非空密码即可 STORE/LOOKUP 成功，登录后重启会经 `/api/v1/refresh` 免密恢复。密钥环处于锁定态时 libsecret 会在平台线程上同步等待解锁，首帧永不产生；Linux runner 原先只在首帧回调里 `gtk_widget_show`，窗口会停在 `IsUnMapped`、应用完全不可见（不是「停在启动加载态」）。`linux/runner/my_application.cc` 已改为启动即 `gtk_widget_show(window)`，提示框挂起期间窗口仍 `IsViewable`，解锁/取消后按降级路径渲染。注意 `dbus-run-session` 里 session bus 只对子进程可见。
  - 编排磁盘守卫（`app/utils/guardrails.py:250`）要求目标路径可用空间 ≥ 1GB 且 ≥ 10%，不满足直接 507；本环境根分区 20G，构建产物堆到 90%+ 后只剩 <1GB/4%，从 UI 或 API 触发新生成必然被挡。要跑生成需先腾出 ~2GB（`/root/.gradle` 约 3.3G、`flutter_client/build/app` 中间产物约 860M、`/root/.pub-cache` 约 630M 是主要可回收项，删除前需用户确认）。
  - headless UI 实测：`apt-get install -y xdotool`。截图用 `python3 -c "from PIL import ImageGrab; ImageGrab.grab(xdisplay=':99').save(p)"`（无需 ImageMagick/xwd），点击与输入用 `xdotool mousemove X Y click 1` / `xdotool type`。无窗口管理器时 `xdotool windowactivate` 因缺 `_NET_ACTIVE_WINDOW` 报错，坐标点击仍有效。验证内置字体是否真的生效：临时用 `FONTCONFIG_FILE` 指向只含 Latin 目录（dejavu/liberation）的自定义 fonts.conf 启动，`fc-list` 确认看不到任何 CJK 字体后再截图（不必卸载系统字体）。字体族在 `app.dart` 里按 `Platform.isLinux` 选择，Android 渲染行为不变；但 pubspec 声明的字体资源会打进所有平台产物，Android APK 同样 +3.1MB；`flutter build bundle` 可单独校验资源打包（比整包构建快得多，磁盘紧张时优先用）。
