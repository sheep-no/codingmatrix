...
- Instructions:
  ...
  - `POST /api/v1/agent/orchestrate/stream` 有磁盘守卫：可用空间 <1GB 或可用率 <10% 直接返回 507（`app/utils/guardrails.py:250`），且会写 `./projects`。跑 agent 端到端生成前需 >2GB 可用，本机根分区 20G、常年 92% 占用，需先腾空间。
  - 编排端到端要跑到 `done`，需求与产物须避开 `unknown` 文件类型：`.py` 在 `EXTENSION_TYPE_MAP` 无兜底（`app/agent/dependency_rules.py:189`），非常规命名的 `.py` 会被推断为 `unknown`，`_infer_unknown_file_types` 随即 `RuntimeError` 中止整轮（`spec_first_generate.py:1955`）。用 `.html/.css/.js/.md` 这类有兜底扩展名的静态站需求即可稳定跑到 `done`（实测 4/4 文件、`success=true`）。真实运行时架构师角色是 `Qwen/Qwen3-8B`（`data/unified_model_config.yaml:356`），弱模型对简单需求会幻觉出 Web 架构或多余文件（如静态站里生成 `app/command.py`）。
