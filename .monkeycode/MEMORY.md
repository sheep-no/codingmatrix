...
- Instructions:
  ...
  - Android release 构建已实测通过：`flutter build apk --release --no-pub` 增量约 3 分钟、全量约 14 分钟，产出 `build/app/outputs/flutter-apk/app-release.apk`（57.9MB，含 arm64-v8a/armeabi-v7a/x86_64）；只需验 Dart 编译时用 `flutter build bundle --target-platform android-arm64`（快得多），验原生/Kotlin 用 `./gradlew :app:compileDebugKotlin --offline`。核对项：`namespace == Kotlin package == 源码目录路径`，Manifest 用 `.MainActivity` 相对 namespace 解析。环境无设备/模拟器且无 KVM、无 vmx/svm，真机项只能静态核对。
