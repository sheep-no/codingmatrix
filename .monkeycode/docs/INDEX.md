...
- PPT 生成链路已完成共享产物卷、格式严格分流、HTML 输出转义、分块上传，以及五类叙事角色和九套独立主题构图；Celery 与旧异步入口统一保存大纲版本、质量阶段、PPT 产物、预览、布局元数据和质量报告；场景识别、模板候选、版本化设计令牌和语义布局评分已接入；Web 三步流程支持大纲新增、删除、重排、编辑与批准校验，预览页展示逐页质量分、问题和修复动作；前端全量测试为 `36 passed`。
