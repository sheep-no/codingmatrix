...
VS Code 工作台由 `vscode-extension/src/agent-workbench.ts` 提供原生 Webview（面板 HTML 在 `workbench-html.ts`），由 `extension.ts` 创建 Agent Host 运行时。工作台分为对话、会话历史、模型、文件版本和性能五个面板：对话支持需求输入、流式事件展示、暂停、恢复、取消、动作批准和拒绝；其余四个面板通过 `workbench_request` / `workbench_response` 通道，由 `workbench-requests.ts` 校验参数后经 `CloudConnection` 读取普通用户 v1 接口（`/api/v1/history`、`/api/v1/conversation/history`、`/api/v1/code/history`、`/api/v1/models/agent-config`、`/api/v1/agent/token-usage`、`/api/v1/agent/snapshots|rollback|snapshot/diff`、`/api/v1/agent/performance`）。Host 通过 `CloudConnection` 与 `/api/v1/agent/host/*` 交互，并通过 `/api/v1/agent/orchestrate/stream` 发起 Agent 流式请求。需要 superadmin 的 `/api/v2/models/*` 与 `/api/v2/model-config/*` 管理面板仍保留在 Web 工作台。
