...
VS Code 工作台由 `vscode-extension/src/agent-workbench.ts` 提供原生 Webview（面板 HTML 在 `workbench-html.ts`），由 `extension.ts` 创建 Agent Host 运行时。工作台分为对话、会话历史、模型、文件版本、性能、学习和设置七个面板：对话支持需求输入、流式事件展示、暂停、恢复、取消、动作批准和拒绝；其余六个面板通过 `workbench_request` / `workbench_response` 通道，由 `workbench-requests.ts` 校验参数后经 `CloudConnection` 读取普通用户 v1 接口（`/api/v1/history`、`/api/v1/conversation/history`、`/api/v1/code/history`、`/api/v1/models/agent-config`、`/api/v1/agent/token-usage`、`/api/v1/agent/snapshots|rollback|snapshot/diff`、`/api/v1/agent/performance`、`/api/v1/agent/learning/stats`、`/api/v1/agent/concurrent-limits/recommended`、`/api/v1/agent/cache/stats|clear`），其中清空全部缓存由扩展侧先弹原生确认。Host 通过 `CloudConnection` 与 `/api/v1/agent/host/*` 交互，并通过 `/api/v1/agent/orchestrate/stream` 发起 Agent 流式请求。需要 superadmin 的 `/api/v2/models/*` 与 `/api/v2/model-config/*` 管理面板仍保留在 Web 工作台。
...
语义规划器 `plan_outline()` 将大纲映射为页面类型、布局候选和容量预算，保留页面及内容块顺序，并对旧版 `title`、`content`、`chart`、`image` 等类型提供兼容映射。规划结果包含布局 ID、布局版本、令牌版本和确定性规划评分；评分结合页面语义、素材需求、内容数量和连续布局惩罚，连续相同布局达到上限时切换兼容候选。商业演示使用 `content_blocks[].metadata` 承载指标、成本、周期、风险、依据、交付物、门槛、负责人和时限；`narrative_role` 将页面归一为机会、证据、策略、路线和决策五类叙事角色。
...
GirlAI 伙伴增强的结构化回合契约位于 `app.schema.girl_companion`，解析和降级入口位于 `app.services.girlai_companion_service.parse_companion_turn`。当前实现支持 JSON、JSON code fence、推理文本后嵌入的完整 JSON 对象和纯文本降级；结构化生成使用独立系统模板、至少 512 个输出 token、最高 0.3 的温度和 60 秒独立请求超时，提供情绪、意图、记忆候选、模型上下文和能力降级字段。`app.services.girlai_companion_classifier` 通过 `DynamicModelRouter` 选择轻量分类模型和 fallback，将输出归一到有限情绪与对话意图标签；低置信度结果使用中性策略，需要关怀的情绪会返回关怀语气和最多三个文字建议。分类失败保留文字主链路，并记录情绪与意图分类降级。当前伙伴能力保持纯对话，不调用工具、不创建任务和不触发提醒。
语音适配使用 `VoiceTranscriptionRequest` 接收已标准化的转写结果，再复用 `generate_companion_turn()` 完成上下文、授权、分类、legacy/unified 双写和幂等处理。`VoiceInputState` 与 `VoiceOutputState` 关联同一 `turn_id`；当前输出供应商能力未接入时声明 `unavailable` 并记录 `voice_output` 降级，文字回复持续可用。

伙伴上下文构建位于 `app.services.girlai_companion_context`，按角色设定、摘要、授权记忆、近期消息和当前输入组合上下文，并在超出预算时保留角色指令和当前输入。`app.services.girlai_companion_model.CompanionModelService` 通过 `DynamicModelRouter` 暴露对话、分类、记忆和 fallback 模型选择；模型调用编排和状态写入仍待后续阶段接入。

结构化伙伴 API 位于 `app.api.v1.GirlAi`，通过 `CompanionTurnRequest` 和 `CompanionTurnResponse` 暴露回合状态与能力降级字段。伙伴回合开始前在 `session_events` 创建短事务 `companion.turn.processing` 预留，`session_id + turn_id` 唯一约束阻止并发重复调用；完成或降级结果按原 sequence 更新并支持响应回放，失败更新为脱敏的 `companion.turn.failed`。processing 预留使用可配置租约，默认 90 秒，中断后的同一回合可原子接管；每次预留生成 reservation token，最终事件通过 token 和 processing 状态执行条件更新，失去租约的旧 worker 会触发事务回滚。`sessions` 使用 `user_id + module + external_id` 唯一约束，保证首次并发请求共享同一 GirlAI 会话。成功回合的记忆候选、legacy `chat_histories`、unified `sessions/messages` 和最终事件共享事务边界。`app.services.girlai_companion_memory.CompanionMemoryService` 将模型结果保存为待确认候选，用户确认后才允许 `companion_allowed` 记忆进入上下文；删除采用 `deleted` 状态和可见性撤销，保持审计记录并立即停止检索。工具审批和任务事件属于后续阶段。
