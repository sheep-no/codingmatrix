...
- 验收：用户从 Web 或 VS Code 工作台完成模型策略同步、Skill 下发与撤销、本地动作审批、验证开关控制、会话暂停/恢复/取消、执行进度展示和结果驱动的 Agent 继续执行，并可从另一工作台恢复同一会话。验收证据：后端单元测试 `1723 passed, 2 skipped`，VS Code 插件构建与测试 `55 passed`。
