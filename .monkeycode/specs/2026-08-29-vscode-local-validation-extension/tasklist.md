...
- 当前进度：已实现 `agent-host.ts` 的 Envelope 解析、Host Hello 生成与解析、会话握手、能力声明和单调递增策略版本门禁；已实现 `ToolDispatcher` 的文件、终端、诊断和验证动作分发，`ApprovalBridge` 的审批请求与决定，以及 `WebviewBridge`、`AgentHostRuntime` 的云端动作轮询、策略更新、原生 Agent Webview 工作台和 activation 命令，VS Code 插件测试 49/49 通过。
