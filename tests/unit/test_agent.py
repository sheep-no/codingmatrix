...
class TestEnhancedExecutor:
    ...
    async def test_execute_write_file(self, tmp_path):
        ...
        result = await executor._tool_write_file({
            "path": str(test_file),
            "content": "Hello, World!"
        })
    ...
    async def test_execute_read_file(self, tmp_path):
        ...
        result = await executor._tool_read_file({"path": str(test_file)})
        assert result is not None
    ...
    async def test_execute_list_files(self, tmp_path):
        ...
        result = await executor._tool_list_files({"directory": str(tmp_path)})
        assert result is not None
...
class TestAgentIntegration:
    ...
    async def test_executor_write_and_read(self, tmp_path):
        ...
        # 写入
        write_result = await executor._tool_write_file({
            "path": str(test_file),
            "content": content
        })
        ...
        # 读取
        read_result = await executor._tool_read_file({"path": str(test_file)})
