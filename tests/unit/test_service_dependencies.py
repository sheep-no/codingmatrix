...
class TestServiceConfigTemplates:
    ...
    def test_detect_services_from_requirement(self):
        ...
        assert "mongodb" in detect_services_from_requirements("MongoDB 文档数据库")
        assert "rabbitmq" in detect_services_from_requirements("消息队列 RabbitMQ")
        assert "elasticsearch" in detect_services_from_requirements("Elasticsearch 搜索")
...
class TestDynamicPackageManager:
    ...
    def test_static_whitelist_contains_common_packages(self):
        """测试静态白名单包含常见包"""
        from app.utils.dynamic_package_manager import STATIC_WHITELIST
        essential = ["fastapi", "redis", "psycopg2-binary", "pymongo", "sqlalchemy", "pytest"]
        for pkg in essential:
            assert pkg in STATIC_WHITELIST
    ...
    def test_heuristic_evaluate_safe_package(self, pkg_mgr):
        """测试启发式评估 - 安全包"""
        result = pkg_mgr._heuristic_evaluate("redis-locks")
        assert result.is_safe is True
        assert result.risk_level in ("safe", "low_risk")

    def test_heuristic_evaluate_suspicious_package(self, pkg_mgr):
        """测试启发式评估 - 可疑包（名字混淆）"""
        result = pkg_mgr._heuristic_evaluate("urllib")
    ...
    def test_dynamic_whitelist_persistence(self, pkg_mgr):
        ...
        assert mgr2.is_in_whitelist("test-new-package") is True
...
class TestDependencyGraphServiceRelations:
    ...
    def test_service_depends_on_env(self):
        """测试 service 类型依赖 env 类型"""
    ...
    def test_service_config_path_type_rules(self):
        """测试 service_config 路径规则"""
        from app.agent.dependency_graph import DependencyGraph
        dg = DependencyGraph()
        matched = [p for p, t in dg.PATH_TYPE_RULES if t == "service_config"]
