...
def test_plaintext_login():
    ...
    if response.status_code == 200:
        ...
        print(f"   ⚠️  登录失败（可能是用户不存在）: {response.status_code}")
...
if __name__ == "__main__":
    ...
    print("测试总结")
    print("="*60)
    print(f"加密登录：{'✅ 通过' if encrypted_success else '❌ 失败'}")
    if plaintext_result is not None:
        print(f"明文登录：{'✅ 通过' if plaintext_result else '⚠️  用户不存在'}")
