...
for schema_path in schemas_to_test:
    try:
        ...
        print(f'  ⚠️  {schema_path}: {str(e)[:80]}')
...
if total_failed == 0:
    ...
    print(f'   ⚠️  {total_failed} 个 API 模块失败，建议检查')
    exit_code = 1
else:
    print(f'   ❌ {total_failed} 个 API 模块失败，不建议部署')
    exit_code = 2
