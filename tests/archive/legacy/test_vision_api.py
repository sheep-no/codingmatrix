...
try:
 ...
 print(f' [FAILED] Vision API 未注册')
except Exception as e:
 print(f' [FAILED] 路由注册测试失败：{e}')
...
try:
 ...
 print(f' [FAILED] Base64 格式错误')
 
 # 清理
 test_path.unlink()
 
except ImportError:
 print(' [WARNING] PIL/Pillow 未安装，跳过图片测试')
except Exception as e:
 print(f' [FAILED] 辅助函数测试失败：{e}')
...
async def test_error_handling():
 ...
 print(' [FAILED] 应该抛出 FileNotFoundError')
 return False
 except Exception as e:
 ...
 print(f' [FAILED] 错误类型不符：{e}')
 return False

try:
 result = asyncio.run(test_error_handling())
except Exception as e:
 print(f' [FAILED] 错误处理测试失败：{e}')
