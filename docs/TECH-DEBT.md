...
- 后端 unit/integration 最近完整记录：`4973 passed, 2 skipped, 0 failed`（186s，master `739256d5` 实测）。较上一记录 `5022 passed` 少约 49 项，主要来自并发合入的 PR #326（清理零引用死代码）删除 `tests/unit/test_multi_language_parser.py`（该文件含 52 个 test 定义，覆盖已删的 `app/agent/multi_language_parser.py`）及其清理的 `app/test/` 历史脚本，非测试能力退化。`--cov=app` 门禁门槛 `58%`，最近一次成功汇总覆盖率 `63.92%`；本机 `make test-cov` 收尾会因工作区陈旧的 `.coverage.*` 并行数据报 `Can't combine statement coverage data with branch data`，CI 全新环境不受影响。`test_process_guard_restart` 在高负载下偶发失败，单跑 `5 passed`。
