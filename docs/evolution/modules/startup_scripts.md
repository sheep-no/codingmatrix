...
- **SS2 [P2] `start.sh` 健康检查访问未挂载的 `/health`，导致 Gunicorn 启动成功也会被报告为失败**——`start.sh:86-100` 监听 8080，`start.sh:102-108` 与 `:172-177` 请求 `http://127.0.0.1:8080/health`；应用实际健康路由是 `/api/v1/health`（`app/main.py:331-332`，路由自身定义于 `app/api/v1/health.py:25`）。`set -e` 下 `start_api` 返回 1 会中止 `start`/`restart` 流程。修复方向：统一健康 URL，并让状态检查与容器 healthcheck 使用同一端点。
