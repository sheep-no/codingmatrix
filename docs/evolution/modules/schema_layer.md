...
> **影响面修订（第 153 轮 v1_api_final.md 交叉）**：auth.py:83 `body: dict` 明文登录直收 JSON，完全绕过 schema 校验——UserLogin(min_length=8) 从不生效，加密登录解密后同样无长度校验。「管理员 6 位密码被明文端点 422 锁死」的表述不成立，实际影响是**登录链密码强度校验整体缺失**（注册链 :234 UserRegister + :241 validate_password_strength 有效）。P2 定级维持，影响面扩大：弱密码账号可正常登录。
