...
- FEBOOT-03 本次修复：`src/main.js:32`、`src/composables/useAuth.js:11-13`、`src/components/leftlist.vue:708` 三处启动路径都会调用 `restoreUser()`，原 `src/utils/tokenManager.js` 的 `refreshAccessToken` 每次调用都新发请求，token 缺失或过期时会并发触发多次 `/api/v1/refresh`。现改为单飞（single-flight）：进行中的刷新由模块级 `inFlightRefresh` 复用，完成后清空以便后续重新刷新。新增 `src/utils/tokenManager.test.js` 断言三个并发调用只发出一次 `/refresh`；回退源码后该断言得到 3 次请求而失败。
- FEBOOT-04 本次修复：`src/composables/useAuth.js` 的 `register` 原请求 `/auth/register`，经 `apiUrl` 前缀拼成 `/api/v1/auth/register`，与后端 `POST /api/v1/register` 不符，已改为 `/register`；`updateProfile` 对应 `/auth/profile`，后端仅有 `GET /api/v1/user/profile`、无更新端点，且零生产消费方，按死代码删除。`useAuth` 的唯一生产消费方 `LoginDialog.vue` 仅使用 `login`，删除不影响现有功能。新增 `src/tests/use-auth-contract.test.js` 断言 register 走 `/register`、`updateProfile` 不再导出；回退源码后两项均失败。
- FEBOOT-05 本次修复：`src/App.vue` 的 `.app-container` 原连续声明 `background: var(--bg-secondary)` 与 `background: var(--gradient-bg)`，第二条始终覆盖第一条，第一条永不生效；`--gradient-bg` 在 `.theme-light`/`.theme-default`/`.theme-dark`/`:root`/深色媒体查询均有定义，故当前无运行时影响，属无效声明。现合并为单条 `background: var(--gradient-bg, var(--bg-secondary))`，行为等价并在变量缺失时回退纯色。新增 `src/tests/app-shell-background.test.js`；回退源码后失败。
