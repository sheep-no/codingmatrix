...
- **TQ6 [P3] 已修复**：`_merge_task_runtime_state` 原直接 `celery_state.lower()` 透传，Celery 的 `FAILURE`/`RETRY`/`REVOKED`/`STARTED` 会变成 `failure`/`retry`/`revoked`/`started`，超出 `TaskStatusEnum` 声明词表（前端 `retry`/`revoked` 落到默认 `queued`，任务失败/被撤销时状态显示错误）。新增 `_CELERY_STATE_TO_STATUS` 归一化：pending/received→pending、started→running、retry→retrying、success→success、failure→failed、revoked→cancelled；未识别的 Celery 状态保留 DB 口径，持久化终态（success/failed/cancelled）优先不变。回归测试 `tests/unit/test_task_dispatch_contract.py` 新增 10 项（7 项状态映射参数化 + 词表约束 + 未知状态回退 + 终态优先）
- **TQ7 [P1] 已修复**：原条目「cancel 白名单含 retrying，DB 层无此值」在**读路径**上不成立（`TaskStatusEnum.RETRYING` 确实存在），但**写路径存在更严重的同源缺陷**：`celery_app.py` 的 `task_postrun` 把 Celery 原生 `state.lower()` 直接落库（`failure`/`retry`/`revoked`），且时序上晚于 `task_failure`/`task_retry` 覆盖其写入的 `failed`/`retrying`；`task_prerun`/`task_failure` 无终态守卫，`acks_late=True` 重投递会把已终态任务改回 `running`。后果是 retry 要求 `status == "failed"`、recover 要求 `failed|cancelled`、cancel 要求 `pending|running|retrying`，在真实失败/重试路径上确定性返回 400，且结果后端 7 天过期后 `failure` 不再被识别为终态。现把映射表与终态集合收敛到 `app/models/task.py` 单一来源（`CELERY_STATE_TO_TASK_STATUS`/`TERMINAL_TASK_STATUSES`），写路径统一走 `_sync_set_task_status` 做归一化 + 终态守卫，读路径改用同一映射。回归测试 `tests/unit/test_celery_signal_status_mapping.py`（13 项）
