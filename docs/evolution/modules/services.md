...
- **AKM3 已修（2026-09-22）**：API Key 原先以明文 `setex` 落 Redis，docstring 的「安全存储」名不副实。修复复用已存在的加密设施（把 `github_config_service` 里的 RSA+AES-GCM 信封抽到 `app/utils/crypto.py` 的 `encrypt_secret`/`decrypt_secret`，github 侧改为薄封装，信封格式保持逐字节不变，存量 GitHub 记录无需迁移）：`store_key` 写入 `v1:` + 信封；`get_key` 只对带 `v1:` 前缀的值解密，无前缀的历史明文原样返回。附加认证数据绑定 `apikey:{user_id}:{token}` 防跨条目重放；解密失败记 error 并返回 None（不泄露异常细节）。存量明文 Key 因全部带 TTL（最长 30 天）自动过期，无需迁移任务。回归 `tests/unit/test_apikey_manager_secret_encryption.py`（3 项：信封落盘不含明文且可解回、历史明文透传、损坏信封返回 None 且记 error）。
...
- **PM1（收集-暴露断链）已修**：`generate_metrics_text()` 原先只放行 `http_requests_total` 前缀的 counter、直方图段固化为空，且 `get_all()` 根本不返回 histograms。现三处一并处理：(1) `HistogramBucket` 之外新增 `Histogram` 数据类保存 labels/`sum`/`count`，`_histograms` 由 `List[HistogramBucket]` 改为 `Histogram`，`get_all()` 返回带桶数组、sum、count 的 histograms；(2) `generate_metrics_text()` 按指标名分组，输出全部 Counter / Gauge / Histogram，histogram 按 Prometheus 规范生成 `_bucket{le=...}` / `_bucket{le="+Inf"}` / `_sum` / `_count`（`le` 用 `_format_float` 去多余小数点）；(3) 由此 `celery_tasks_total`、`python_gc_objects_collected`、`database_connections_active` 不再收集后静默丢弃。附带清理 `PrometheusMetrics.__init__` 的 `_request_counts`/`_request_durations`/`_lock` 三个零引用死字段，并修复原先 `http_requests_total{...}{...}` 双标签块非法行。新增回归 `tests/unit/test_prometheus_metrics_exposure.py`（3 项：直方图桶/sum/count、非 HTTP counter、database gauge），原 `tests/unit/test_performance_monitor_metrics.py` 断言保持通过。
