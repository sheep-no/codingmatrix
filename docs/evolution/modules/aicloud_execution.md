...
- **CA6 已修复（2026-09-22）**：`MALICIOUS_PATTERNS` 三处实测漏检/误伤已收敛。① `os\.system`/`os\.popen`/`socket\.socket`/`pty\.spawn` 只认紧邻点号，而 `os .system(...)`、`os.  system(...)` 是合法 Python 且能执行，现改为 `os\s*\.\s*system` 等；② `subprocess\.call` 只覆盖 `.call`，`subprocess.run` / `Popen` / `check_output` / `check_call` 全部漏检，现改为 `(?:call|run|Popen|check_call|check_output)`；③ `rm\s+-rf\s+/` 只匹配目标以 `/` 开头，`rm -rf ~`、`rm -rf *`、`rm -rf $HOME`、`rm -fr /` 漏检，现按 `rm\s+-[a-z]*r[a-z]*f\s+[~/*$.]`（含 `-fr` 顺序）匹配。同时删除 `base64\.b64decode\s*\(`：Base64 解码是图片 data URL 与二进制载荷的正常处理路径，单凭该调用判定恶意会造成稳定误报。新增 `tests/unit/test_aicloud.py::TestContentAnalyzer` 15 项（11 条绕过变体命中 + 4 条正常内容不误报），回退 `content_analyzer.py` 后 13 项失败。
- **CI3 已修复（2026-09-22）**：`get_isolator` 的单例判断无锁，而 `ContextIsolator` 持有 `sandbox_env`/`main_project_env` 可变状态——并发首次调用会各建一份实例，`setup_sandbox` 写入的实例与后续读取的实例可能不是同一个，沙箱环境读不到。现按项目既有单例范式（`health_checker`/`system_config`）加双检锁。新增并发回归（8 线程 + 构造内 `sleep` 放大竞态窗口），回退 `context_isolator.py` 后构造计数为 8 而非 1。
- **CA7 待确认**：`check_dangerous_extensions` 与 `SAFE_FILE_EXTENSIONS` 在 `app/` 内零生产消费（仅 `tests/unit/test_aicloud.py` 引用），文件类型检查形同虚设；删除涉及测试同步调整，待确认后执行。
