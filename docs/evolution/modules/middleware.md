...
- **IV6 [P3] 已修复（MIME 主/子类型判定）**：原判定用 `content_type.startswith(allowed)` 与 `content_type.startswith("application/json")` 两处前缀匹配，同时造成漏判与误判——`application/vnd.api+json`（结构化后缀）不在白名单又不以 `application/json` 开头，被 415 拒绝；`application/jsonp` 因前缀命中被当作 JSON，非 JSON body 会得到 400 而非 415。现抽出 `_mime_type()`（先剥离 `;charset=`/`;boundary=` 参数）与 `_is_json_content_type()`（`application/json` 或 `+json` 后缀），白名单 `/415` 判定、JSON 扫描判定、`_size_limit_for()` 的 multipart 判定三处统一走 MIME 主类型，不再依赖前缀。
- **P2-3 [P2] 已修复（中间件体积上限与上传配额冲突）**：原中间件对 `POST/PUT/PATCH` 一律按 `MAX_BODY_SIZE=10MB` 判 `Content-Length`，导致 10-100MB 的**整体**上传（`multipart/form-data`）在到达上传端点前就被 413，而 `file_upload` 端点的 `MAX_FILE_SIZE` 与 `Settings.MAX_UPLOAD_SIZE_MB` 为 100MB（分片上传 `CHUNK_SIZE=5MB` 因单请求小于 10MB 未受影响）。现新增 `MAX_UPLOAD_BODY_SIZE = settings.max_upload_size_mb * 1024 * 1024` 与 `_size_limit_for(content_type)`：`multipart/form-data` 按上传配额放行，其余类型仍按 10MB 严格限制；`file_upload.MAX_FILE_SIZE` 同步改为读 `settings.max_upload_size_mb`，两侧共用单一配置。`_read_body_safe` 改为接收 `limit` 参数，读取阶段与声明校验阈值一致。
- **P2-3 测试**：`tests/unit/test_input_validator.py` 新增 4 项（multipart 50MB 放行、multipart 超配额 413、JSON 20MB 仍 413、中间件与上传端点配额一致）。
- **IV6 测试**：`tests/unit/test_input_validator.py` 新增 3 项（`application/vnd.api+json` 含注入被 400 拒绝、`application/vnd.api+json; charset=utf-8` 合法 body 透传、`application/jsonp` 非 JSON body 改判 415）；回退 `input_validator.py` 后 3 项失败（415 判定与 JSON 判定分别在两处前缀命中）。
