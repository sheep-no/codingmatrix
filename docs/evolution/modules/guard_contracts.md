...
- **GC1 [P2] 已修**：`check_file` 新增可选 `original_content`。existence 检查现仅在
  ...
  消除对任意变更后文件的假阳性。
- **GC3 [P3] 已修**：`check_file` 在需要时用 AST 提取函数/类签名（参数列表 / 基类），
  提供基线时比对签名，识别「函数仍在但参数被改」；无基线时保留原存在性检查语义。
- **GC6 [P3] 已修**：`_pattern_present` 对纯标识符保护项（`^[A-Za-z_]\w*$`）改整词匹配，
  `id` 不再命中 `identifier` 等子串（`@router.get` 等非标识符模式仍走子串匹配）。
- **注意（`app/tasks/code_tasks.py:186`）**：该调用点只有变更后内容，未传
