...
- **部署层已修（本轮）**：`docker-compose.prod.yml` 原先 `api`/`celery`/`scheduler` 都未挂载 `uploads`，`UPLOAD_ROOT = Path("./uploads").resolve()` 在各容器内各自解析为容器可写层里的 `/app/uploads`，scheduler 看不到 api 写入的上传文件，且容器重建即丢。现统一四类受管目录为共享命名卷并挂载到三者：`uploads:/app/uploads`、`ppt-artifacts:/app/pptx_output`、`generated-images:/app/generated_images`、`projects:/app/projects`（`ppt-artifacts` 原先 api/celery 已挂、scheduler 缺失；后两者此前任何服务都未挂载）。后两者覆盖 `app/utils/image_generation.py::OUTPUT_DIR`（Kolors 图按龄淘汰）与 `app/core/paths.py::PROJECTS_BASE_DIR`（`unified_retention_task` 经 `LocalProjectStorageAdapter` 物理删除项目目录）——缺挂载时 `delete_project` 因 `path.exists()` 恒为 False 而返回 `already_deleted`，数据库记录被推进但 api 容器里的项目文件残留。新增结构断言 `tests/unit/test_compose_structure.py::test_prod_compose_shares_cleanup_volumes_with_scheduler`（覆盖三服务 × 四挂载 + 卷定义）。
- **调度器长周期任务永不执行（本轮已修）**：核实 `IntervalTrigger` 的 7/10/1 天任务均未传 `start_date`，APScheduler 3.11.3 的 `__init__` 会以 `start_date or (now + interval)` 兜底，实测 `chat_archive` 的 `start_date` 为 `2026-10-01`（10 天后）。调度容器 `restart: always` 且每次发版重建，重启周期远短于 7/10 天，文件清理、任务清理、日志清理、对话归档、项目保留与生成物淘汰等任务实际从不执行。现为这些任务传入 `start_date=_deferred_start()`（默认启动后 60s），首次触发落在启动后不久，后续仍按原间隔重复；每分钟的 worker lease 恢复任务保持默认。新增回归 `tests/unit/test_scheduler_job_start_schedule.py`（2 项）。

## 10. 状态更新（2026-09-26 死代码清理）

- **`create_tables()` 死函数已删除**：DB12 说明中「仍未做」的 `app/main.py::create_tables()` 零调用函数（docstring 称「保留以备不时之需」）已从 `app/main.py` 移除，随删的 `from app.models.base import Base` 与 `db.database` 中的 `engine` 导入（两者仅被该函数使用，`Base` 在 `migrations/env.py`、`migrations/runner.py`、`app/models/*` 有独立导入，模型注册不受影响）。运行时建表唯一路径仍是 `migrations/runner.py::run_async_migrations()`。schema 单轨收敛仍未做。
