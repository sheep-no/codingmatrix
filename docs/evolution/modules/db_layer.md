...
- **部署层已修（本轮）**：`docker-compose.prod.yml` 原先 `api`/`celery`/`scheduler` 都未挂载 `uploads`，`UPLOAD_ROOT = Path("./uploads").resolve()` 在各容器内各自解析为容器可写层里的 `/app/uploads`，scheduler 看不到 api 写入的上传文件，且容器重建即丢。现新增 `uploads` 命名卷并挂载到三者，同时补齐 scheduler 缺失的 `ppt-artifacts:/app/pptx_output`（api/celery 早已挂载，缺它则生成物按龄淘汰扫到空目录）。新增结构断言 `tests/unit/test_compose_structure.py::test_prod_compose_shares_cleanup_volumes_with_scheduler`。**仍存**：`app/utils/image_generation.py` 的 `OUTPUT_DIR = Path("./generated_images")` 未挂载任何卷，api 写入的 Kolors 图留在容器层、scheduler 侧不可见，`DEFAULT_IMAGE_OUTPUT_DIR` 的按龄淘汰实际覆盖不到；需确认是否同样挂共享卷后再改。
