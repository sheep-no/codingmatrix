...
> 最后更新：2026-09-22 | 后端：423 个 Python 文件 / 117,655 行 | API：28 个挂载路由 / 约 279 个路由装饰器 | Agent：125 个 Python 文件 | 数据模型：34 张表 | Alembic：15 个有效迁移文件 | Flutter：70 个 Dart 文件 / 12,226 行

---

## 架构概览

当前架构在统一 LLM 调用路径之外，已经形成统一状态层、PPT 阶段编排、模型上下文持久化、Mobile Agent、VS Code Agent Host 和 Flutter 客户端等运行时能力。
...
| VS Code 扩展 | `vscode-extension/src/` 共 21 个 TypeScript 文件，流式生成使用 `/api/v1/agent/orchestrate/stream`，工作台含对话、会话历史、模型、文件版本、性能、学习和设置七个面板，支持本地结果恢复 |
| Flutter 客户端 | `flutter_client/` 共 70 个 Dart 文件 / 12,226 行，18 个页面；经 `AuthenticatedClient` 调用现有 v1/v2 API，MCP 管理仅 superadmin |
